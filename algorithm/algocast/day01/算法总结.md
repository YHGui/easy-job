...
34. 大数加/乘法
    ...
    public class Solution {
        public String add(String s1, String s2) {
            char[] ch1 = new StringBuilder(s1).reverse().toString.toCharArray();
            char[] ch2 = new StringBuilder(s2).reverse().toString.toCharArray();
            ...
            for (int i = 0; i <= maxLen; i++) {
                int val1 = i < len1 ? (ch1[i] - '0') : 0;
                int val2 = i < len2 ? (ch2[i] - '0') : 0;
                result[i] = val1 + val2;
            }
        ...
        //Karatsuba快速相乘算法, 时间复杂度为O(nlog3)
        public karatsuba(long num1, long num2) {
            if (num1 < 10 || num2 < 10) {
                return num1 * num2;
            }
            ...
            return (long)(z1 * Math.pow(10, 2 * half) + z2 + z3 * Math.pow(10, half));
        }
        ...
        public karatsuba(long num1, long num2) {
            ...
            int half = n / 2 + n % 2;  //另一种划分方法

            //返回num1 / half 和 num1 % half
            BigInteger[] a_b = num1.divideAndRemainder(BigInteger.valueOf(10).pow(half));
            BigInteger a = a_b[0];
            BigInteger b = a_b[1];
            BigInteger[] c_d = num2.divideAndRemainder(BigInteger.valueOf(10).pow(half));
            BigInteger c = c_d[0];
            BigInteger d = c_d[1];
            ...
            BigInteger res = step1.multiply(BigInteger.valueOf(10).pow(2*half)).add(step2)
                    .add(step4.multiply(BigInteger.valueOf(10).pow(half)));
...
解决办法:程序的开始把前k个元素都放到水库中,然后对之后的第i个元素,以k/i的概率换掉这个水库中的某一个元素,以此类推，以k/m的概率选择第m个对象（m>k）。如果m被选中，则随机替换水库中的一个对象。最终每个对象被选中的概率均为k/n，证明如下:第m个对象被选中的概率=选择m的概率*（其后元素不被选择的概率+其后元素被选择的概率*不替换第m个对象的概率）
...
37. 两个栈实现队列
    ...
    // pop的话如果out栈没有元素,先将in栈中数全数push到out栈中,最后在out栈顶的就是第一个入in栈的,如果in和out栈都有元素,out栈的栈顶是目前全局最先进入栈的数
        public int pop() {
            transferIfEmpty();
            return out.pop();
        }
    //判断是否empty,只要判断两个栈是否都empty
...
39. 恢复IP地址 **Restore IP Addresses**
...
40. 完全二叉树判断
    ...
    public class Solution {
        public boolean isCompleteBinarySearchTree(node root) {
            // Write your code here
            ...
            while(!q.isEmpty()) {
                ...
                for(int i = 0; i < size; i++) {
                    node p = q.poll();
                    if(p.right != null && p.left == null || flag == 1 && (p.left != null || p.right != null)) {
                        return false;
                    }
...
42. LCS
    ...
    public class Solution {
        public int LCS(String A, String B) {
            int n = A.length();
          	int m = B.length();
          //f[i][j] 表示A 和 B的LCS长度，如果A的第i - 1个值和B的第j - 1个值不等，则求max{f[i - 1][j], f[i][j - 1]}，否则如果相等，则为f[i - 1][j - 1] + 1
          	int f[][] = new int[n + 1][m + 1];//初始化数组值都为0
          	for (int i = 1; i<= n; i++) {
                for (int j = 1; j <= m; j++) {
...
follow up 1：求出任意一个LCS

```java
private String getAnyOneLCS_NonRecursive(String A, String B, int[][] f) {
    StringBuilder sb = new StringBuilder();
...
follow up 2: 求出所有的LCS

```java
private Set<String> getAllLCS(String A, String B, int[][] f) {
    Set<String> results = new HashSet<>();
  	dfs("", A.length(), B.length(), A, B, f, results);
  	return results;
}
// 递归的定义
private void dfs(String s, int i, int j, String A, String B, int[][] f, Set<String> results) {
  //递归的出口
    if (i == 0 || j == 0) {
    ...
    //递归的拆解                
    if (A.charAt(i - 1) == B.charAt(j - 1)) {
        dfs(s + A.cahrAt(i - 1), i - 1, j - 1, A, B, f, results);
    } else {
        if (f[i - 1][j] >= f[i][j - 1]) {
            dfs(s, i - 1, j, A, B, f, results);
        }
      	
      	if (f[i][j - 1] >= f[i - 1][j]) {
            dfs(s, i, j - 1, A, B, f, results);
        }
    }
}
```

43. 旋转字符串:思路：三步反转法XY（比如X = ‘abc’， Y = ‘defg’）组成字符串，需要变成YX，即变成了“defgabc”，将(X^TY^T)^T = YX
...
44. 将 *k* 个有序数组合并为一个大的有序数组

    思路:可以用堆做到 O(N log k) 的时间复杂度.初始将所有数组的首个元素入堆, 并记录入堆的元素是属于哪个数组的.每次取出堆顶元素, 并放入该元素所在数组的下一个元素.
...
1. 搜索二维矩阵:二维数组可以转化为一维有序数组,可以使用二分法

   ```java
   public class Solution {
       public boolean searchMatrix(int[][] matrix, int target) {
           if (matrix == null || matrix.lenght == 0) {
               return false;
           }
           int start = 0;
...
2. 搜索二维矩阵2:根据行/列有序,每次对比之后排除掉一行或者一列数
...
1. 二叉树的前序遍历
   ...
   public class Solution {
       ...
       public ArrayList<Integer> preorderTraversal(TreeNode root) {
           ...
           ArrayList<Integer> result = new ArrayList<Integer>();
           //stack for help
           Stack<TreeNode> stack = new Stack<TreeNode>();9930115g
           ...
           while (!stack.empty()) {
               ...
               //right child tree into stack
               if (node.right != null) {
                   stack.push(node.right);
               }
               //left child tree into stack
               if (node.left != null) {
...
2. 二叉树中序遍历,思路:
   ...
   1. 首先访问左子树,将左子树都存入栈中直到左子树为空;
   2. 弹出栈顶元素,加入结果列表,并转向右子树,
   3. 然后同样将其左子树都存入栈中直到左子树为空,重复上述操作
   4. 直到curt节点和stack同时为空
