...
3. 但如果ActiveStandbyElector在异常状态下Zookeeper Session关闭,那么`/hadoop-ha/${dfs.nameservices}/ActiveBredCrumb`是持久节点,会一直保留下来,后来当另一个NameNode选主成功之后,会注意到上一个Active NameNode遗留下来的这个节点,从而会回调ZKFailoverController的方法对就的Active NameNode进行fencing
